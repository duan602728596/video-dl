...
exports.parseWeiboVideo = parseWeiboVideo;
...
async function parseWeiboVideo(videoUrl, videoType, options) {
  // 获取id
  const urlResult = new _url.default.URL(videoUrl);
  const videoId = urlResult.pathname.replace(/\/tv\/show\//i, ''); // 获取视频信息

  const data = (0, _stringify.default)({
  ...
  return {
    ...
    url: getVideoByQuality(playInfo.urls),
    // 挑选最高清的视频
